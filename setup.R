...
# returns vector of logical is prob columns are FALSE and TRUE, else returns vector of factor
  { f = prob[,colnames(prob)==class]>=cutoff
...
#caption = function(x, title=NULL, row.names=FALSE, position=NULL)
...
#captionx = function(x, title=NULL, row.names=FALSE, position=NULL)
#  { if (is.null(title)) title = as.character(sys.call(sys.parent(6)))[2]
#    if (is.null(position)) position = "center"
#    x.fmt = as.character(kable(x, format="html", align=rep("r", ncol(x)), caption=paste0("<", position, "><font color=\"black\"><b>", title, "</b></font></", position, ">"), row.names=row.names)) }
...
# sortby = function(v1, v2, decreasing=FALSE) factor(v1, v1[order(v2, decreasing=decreasing)])
...
var_info.sd = function(data) { x = aaply(colnames(data), 1, function(cn) if (is.numeric(data[,cn])) sd(data[,cn], na.rm=TRUE) else NA)
                                names(x) = colnames(data)
                                x }

var_info.var = function(data) { x = aaply(colnames(data), 1, function(cn) if (is.numeric(data[,cn])) var(data[,cn], na.rm=TRUE) else NA)
                                names(x) = colnames(data)
                                x }

var_info.na_count = function(data) { x = aaply(colnames(data), 1, function(cn) length(which(is.na(data[,cn]))))
                                     names(x) = colnames(data)
                                     x }

var_info.type = function(data) { x = aaply(colnames(data), 1, function(cn) class(data[,cn]))
                                 names(x) = colnames(data)
                                 x }									 
